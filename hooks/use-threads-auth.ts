...
function getLoginApiUrl(): string {
  const redirectUri = process.env.NEXT_PUBLIC_THREADS_REDIRECT_URI || 'https://0cf8138f070a.ngrok-free.app/auth'
  const apiBaseUrl = process.env.NEXT_PUBLIC_THREADS_API_URL || 'https://threads-platform-cms.vercel.app'
  return `${apiBaseUrl}/api/users/login?redirect_uri=${redirectUri}`
}
...
export function useThreadsAuth() {
  ...
  const setRedirecting = (isRedirecting: boolean) =>
    setState(prev => ({ ...prev, isRedirecting }))

  const setError = (error: string | null) =>
    setState(prev => ({ ...prev, error }))

  const sendAuthCode = useCallback(async (code: string): Promise<void> => {
    try {
